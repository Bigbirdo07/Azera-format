...
def render_suggested_questions_panel(loaded, profile, selected_sheet: str,
                                     settings: dict[str, Any]) -> None:
    """Show curated + workbook-tailored questions for the loaded sheet.

    The first tab ("Tailored to your workbook") is generated from the actual
    columns and sniffed values in the active sheet, so a K-12 roster shows
    `Show me 5th graders` while a college sheet shows `Show me freshmen`. The
    remaining tabs come from the static curated library, filtered to columns
    this workbook actually has.

    Auto-expands when the chat is empty so first-time users see options; once
    a conversation has started the panel collapses to stay out of the way.
    """
    if loaded is None or profile is None or not selected_sheet:
        return
    sheet_columns = {sheet.name: sheet.columns for sheet in profile.sheets}
    columns = sheet_columns.get(selected_sheet) or []

    cats: list[AskableCategory] = []
    frame = loaded.sheets.get(selected_sheet) if hasattr(loaded, "sheets") else None
    dynamic = build_dynamic_suggestions(frame, columns)
    if dynamic:
        cats.append(AskableCategory(
            id="dynamic_tailored",
            title="Tailored to your workbook",
            blurb="Suggestions generated from this sheet's columns and values.",
            questions=list(dynamic),
        ))
    cats.extend(askable_categories(columns))
    if not cats:
        return

    has_history = bool(st.session_state.get("chat_messages"))
    with st.expander(f"Suggested questions ({sum(len(c.questions) for c in cats)})",
                     expanded=not has_history):
        st.caption("Click any question to ask it. Only questions answerable with "
                   "this workbook's columns are shown.")
        tabs = st.tabs([c.title for c in cats])
        for tab, category in zip(tabs, cats):
            with tab:
                if category.blurb:
                    st.caption(category.blurb)
                for question in category.questions:
                    if st.button(question.text, key=f"qlib_{question.id}",
                                 use_container_width=True):
                        st.session_state["_clicked_template_id"] = question.id
                        route_message(
...
def render_suggested_actions(loaded, profile, selected_sheet: str,
                             settings: dict[str, Any]) -> None:
    """A short, scannable set of one-click actions (max 6).

    Workbook-tailored suggestions come first (generated from this sheet's real
    columns/values via build_dynamic_suggestions), then curated questions that
    the detected columns can answer. No counts, no expander — just buttons.
    """
    if loaded is None or profile is None or not selected_sheet:
        return
    sheet_columns = {sheet.name: sheet.columns for sheet in profile.sheets}
    ...
    def _add(question) -> None:
        label = (question.text or "").strip()
        key = label.lower()
        if label and key not in seen:
            seen.add(key)
            picks.append(question)

    for question in build_dynamic_suggestions(frame, columns):
        _add(question)
    for category in askable_categories(columns):
        for question in category.questions:
            _add(question)
    ...
    cols = st.columns(2)
    for index, question in enumerate(picks):
        with cols[index % 2]:
            if st.button(question.text, key=f"suggest_{question.id}",
                         use_container_width=True):
                st.session_state["_clicked_template_id"] = question.id
                route_message(
                    request=question.text,
...
def _render_attachment(
    ...
    *,
    ...
    if kind == "result":
        _render_result_card(attachment, message_index, loaded, profile, selected_sheet, settings)
    elif kind == "confirmation":
        _render_confirmation_card(attachment, message_index, loaded, profile, selected_sheet, settings)
    elif kind == "edit_plan":
        _render_edit_plan_card(attachment, message_index)
    elif kind == "download":
        _render_download_card(attachment, message_index)


def render_live_output_panel(loaded, profile, selected_sheet: str, settings: dict[str, Any]) -> None:
    """Right panel content (title supplied by render_card wrapper).

    The on-screen state shifts to "Modified Working Sheet" via a small banner
    once a confirmed edit produced an updated workbook, so the user knows the
    visible view is a derived working copy, not the source roster.
    """
    modified = bool(st.session_state.get("latest_output_file"))
    if modified:
        st.markdown(
            '<div style="font-size:0.82rem; color:#15803D; '
            'background:#DCFCE7; border:1px solid #BBF7D0; '
            'border-radius:8px; padding:6px 10px; margin-bottom:8px;">'
            'Modified Working Sheet · the original workbook is unchanged.</div>',
            unsafe_allow_html=True,
        )

    if loaded is None or not selected_sheet:
        st.markdown(
            '<div class="empty-state"><strong>No working result yet.</strong>'
            '<ul><li>Ask a question in the chat to create a filtered or modified sheet.</li>'
            '<li>Try: <em>Show me Biology students below 2.0 GPA</em></li></ul></div>',
            unsafe_allow_html=True,
        )
        return

    if st.session_state.get("assistant_mode") == "clarify":
        reason = st.session_state.get("clarify_reason") or "Needs clarification"
        question = st.session_state.get("clarify_question")
        with st.container(border=True):
            st.markdown(f"<div style='font-weight:600; font-size:1.05rem; margin-bottom:8px; color:#C2410C;'>Warning: {reason}</div>", unsafe_allow_html=True)
            st.warning(question)
            
            options = st.session_state.get("clarify_options") or []
            if options:
                for index, option in enumerate(options):
                    if st.button(option, key=f"live_clarify_opt_{index}", use_container_width=True):
                        route_message(
                            request=option,
                            selected_sheet=selected_sheet,
                            loaded=loaded,
    ...
    headline = _live_output_headline(messages, latest_index)
    if headline:
...
def _live_output_headline(messages: list[dict[str, Any]], latest_index: int) -> str | None:
    """Surface a short headline above the Live Output card — the user's latest
    question, lightly normalized."""
...
def _render_result_card(
    ...
    # 1. Interpretation & Assumptions (separate from final result)
    assumption_note = attachment.get("assumption_note")
    if assumption_note:
        st.info(f"**Interpretation & Assumptions:** {assumption_note}")
    
    # 2. Columns Used & Active Filter
    columns_used = attachment.get("columns") or []
    if columns_used:
        st.markdown(f"**Columns Used:** " + ", ".join([f"`{c}`" for c in columns_used]))
        
    filters_used = attachment.get("filters") or []
    if filters_used:
        from nlp.conversation import describe_filters
        desc = describe_filters(filters_used)
        st.markdown(f"**Active Filter:** {desc}")

    table = attachment.get("table") or []
    row_count = attachment.get("row_count")
    if isinstance(row_count, int):
        st.caption(f"{row_count} matching record(s)")
    value = attachment.get("value")
    if value is not None and not table:
        operation = (attachment.get("operation") or "").replace("_", " ").title()
    ...
    if table:
        ...
        st.download_button(
            ...
            file_name=f"result_msg{message_index}.csv",
            mime="text/csv",
            key=f"dl_result_{message_index}",
            use_container_width=True,
        )
    if attachment.get("redacted"):
        st.caption("Some sensitive fields are hidden by default.")
    if attachment.get("review_note"):
        st.caption(attachment["review_note"])

    is_latest = _is_latest_result_message(message_index)
    if is_latest:
        _render_chip_row(
            attachment, message_index, loaded, profile, selected_sheet, settings,
        )
...
def _render_chip_row(
    ...
    """Render alternative-interpretation chips (medium band) and next-move chips.

    Each chip click routes its label as the next user message via
    `route_message`. Alternative chips also stash the originating entry id so
    the resulting turn is logged as a correction of *this* interpretation.

    Visual grouping:
      - Alternatives sit in a bordered container under a bold "Did I get that
        right?" heading. They use `type="primary"` (filled) so they read as
        the action that re-asks the question.
      - Next moves sit below in plain outlined buttons under a "Next moves"
        heading — these are forward actions, not corrections.
    """
    alternatives = list(attachment.get("alternatives") or [])
    suggestions = list(attachment.get("suggestions") or [])
    entry_id = attachment.get("entry_id")

    if alternatives:
        with st.container(border=True):
            st.markdown("**Did I get that right?**")
            cols = st.columns(min(len(alternatives), 3))
            for index, alternative in enumerate(alternatives[:3]):
                if cols[index].button(
                    alternative,
                    key=f"alt_{message_index}_{index}",
                    use_container_width=True,
                    type="primary",
                ):
                    if entry_id:
                        st.session_state["_force_correction_target"] = entry_id
                    route_message(
                        request=alternative,
                        selected_sheet=selected_sheet,
                        loaded=loaded,
                    ...
                    st.rerun()
            st.caption(
                "Choosing a different definition helps the assistant learn better rules."
            )
    ...
    if suggestions:
        ...
        for index, suggestion in enumerate(suggestions[:3]):
            if cols[index].button(
                suggestion,
                key=f"sug_{message_index}_{index}",
                use_container_width=True,
            ):
                route_message(
                    request=suggestion,
...
def _render_confirmation_card(
    ...
    options = attachment.get("options") or ["Confirm", "Cancel"]
    confirm_label, cancel_label = (options + ["Confirm", "Cancel"])[:2]
    reason = attachment.get("reason")

    with st.container(border=True):
        st.markdown("<div style='font-weight:600; font-size:1.05rem; margin-bottom:8px; color:#9A3412;'>Confirmation Required</div>", unsafe_allow_html=True)
        if reason:
        ...
        if resolved == "cancelled":
            ...
            return
        if resolved == "expired":
            st.warning("Superseded by a newer request.")
            return

        # Buttons are only active while this is the pending confirmation in memory.
        pending = (st.session_state.get("assistant_memory") or {}).get("pending_action") or {}
        if not pending:
            st.caption("No longer pending.")
            return
...
def _render_edit_plan_card(attachment: dict[str, Any], message_index: int) -> None:
    plan = attachment.get("plan") or {}
    confidence = attachment.get("confidence")
    level = attachment.get("level")
    warning = attachment.get("warning")
    validation_error = attachment.get("validation_error")
    resolved = attachment.get("resolved")

    with st.container(border=True):
        st.markdown("<div style='font-weight:600; font-size:1.05rem; margin-bottom:8px; color:#1E3A8A;'>Planned Workbook Changes</div>", unsafe_allow_html=True)
        if confidence is not None:
            st.caption(f"Confidence: {confidence:.0%} ({level})")
        if warning:
            st.warning(warning)
        if validation_error:
        ...
        commands = plan.get("commands") or []
        if len(commands) > 1:
            st.caption(f"Steps ({len(commands)})")
            for step_index, command in enumerate(commands, 1):
                action = command.get("action", "?")
                sheet = command.get("sheet", "")
                out = command.get("output_sheet", "")
                target = f" → {out}" if out else ""
                st.caption(f"{step_index}. {action} on {sheet}{target}")
        ...
        # The Run / Edit / Cancel buttons act on the LATEST edit plan. Older plans
        # remain visible as a record but their buttons are disabled.
        is_latest = (
            st.session_state.get("assistant_mode") == "edit_workbook"
            and st.session_state.get("current_plan") == plan
        )
        if not is_latest:
            st.caption("(Replaced by a newer request.)")
            return

        st.session_state["_chat_edit_plan_message_index"] = message_index
        can_execute = bool(attachment.get("can_execute"))
        cols = st.columns(2)
        cols[0].button(
            "Run Action",
            type="primary",
            ...
            key=f"edit_run_{message_index}",
        )
        cols[1].button(
            "Cancel",
            use_container_width=True,
            key=f"edit_cancel_{message_index}",
        )


def _render_download_card(attachment: dict[str, Any], message_index: int) -> None:
    path = Path(str(attachment.get("path") or ""))
    if not path.exists():
        st.caption("The exported file is no longer available locally.")
        return
    st.download_button(
        ...
        use_container_width=True,
        key=f"dl_workbook_{message_index}",
    )
...
def _is_latest_result_message(message_index: int) -> bool:
    messages = st.session_state.get("chat_messages") or []
    latest_index = -1
    for index in range(len(messages) - 1, -1, -1):
        attachment = messages[index].get("attachment") or {}
        if attachment.get("type") == "result":
            latest_index = index
            break
    return latest_index == message_index
